...
export default function InvestmentTips() {
    ...
    // variante da animação pro motion
    const item = {
        hidden: { opacity: 0 },
        show: { opacity: 1 }
      }
    ...
    onMouseLeave={()=> setHoverComponent(false)}>
        <div className='w-full h-[50px] flex justify-between items-start'>
            <div className='h-[100%] flex justify-start items-start'>
                <p className='font-semibold mt-[10px] text-[17px] dark:text-[#fff]'>Dicas de Investimento</p>
            </div>
            <div className='flex h-[100%] items-start w-auto'>
                ...
                initial="hidden"
                animate="show">
                    <IoMdArrowDropdown className="text-[25px] mt-[10px] hover:bg-[#eee] rounded-full
                    cursor-pointer dark:text-[#fff]"
                    onClick={handleClick}/>
                </motion.div> : ''}
            </div>
        </div>
